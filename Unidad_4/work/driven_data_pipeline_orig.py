...
s3 = boto3.client('s3')
bucket = 'driven-data-bucket-ert'
key = 'raw_data.csv'


with DAG(
    dag_id='driven_data_pipeline',
    schedule_interval='@daily',
    start_date=datetime(2025, 10, 18),
    catchup=False,
) as dag: start = DummyOperator(task_id='start')
...
def read_csv_from_s3(bucket_name: str, key: str) -> list:
    """
    Lee un archivo CSV desde S3 y devuelve su contenido como lista de filas.
    
    Args:
        bucket_name (str): Nombre del bucket S3.
        key (str): Ruta del archivo dentro del bucket.
    
    Returns:
        list: Lista de filas (cada fila es una lista de columnas).
    """
    s3 = boto3.client('s3')
...
def append_csv_to_s3(bucket_name: str, key: str, headers: list, rows: list) -> None:
    """
    Escribe contenido CSV en un archivo S3, agregando nuevas filas.
    
    Args:
        bucket_name (str): Nombre del bucket S3.
        key (str): Ruta del archivo dentro del bucket.
        headers (list): Lista de encabezados CSV.
        rows (list): Lista de filas a escribir (cada fila es una lista de columnas).
    """
    s3 = boto3.client('s3')
    ...
    # Concatenar contenido si el archivo ya existía
    if file_exists:
        final_content = existing_content + buffer.getvalue()
    else:
        final_content = buffer.getvalue()

    # Subir el contenido actualizado a S3
    s3.put_object(Bucket=bucket_name, Key=key, Body=final_content.encode('utf-8'))
...
def _write_to_csv() -> None:
    ...
    if str(date.today()) == "2025-10-21":
        rows = random.randint(100, 301)
    else:
        rows = random.randint(0, 50)

    data = [_generate_record(fake) for _ in range(rows)]

    append_csv_to_s3(bucket, key, headers, data)
...
def _add_id() -> None:
    ...
    df = pl.read_csv("s3://driven-data-bucket-ert/raw_data.csv")
    ...
    #df.write_csv("s3://driven-data-bucket-ert/raw_data.csv")
    # Log the action.
...
load_raw_data_task = SQLExecuteQueryOperator(
    ...
    FROM 's3://driven-data-bucket-ert/raw_data.csv' 
    DELIMITER ',' 
    CSV HEADER;
    """
)
